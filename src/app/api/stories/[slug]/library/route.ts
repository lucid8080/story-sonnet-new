...
export async function POST(
  ...
  try {
    await prisma.userSavedStory.upsert({
      where: {
    ...
    return NextResponse.json({ inLibrary: true });
  } catch (e) {
    console.error('[library POST]', e);
    return NextResponse.json(
      {
        error: 'Could not add to library.',
...
export async function DELETE(
  ...
  try {
    await prisma.userSavedStory.deleteMany({
    ...
    return NextResponse.json(
      {
        error: 'Could not remove from library.',
        debug: e instanceof Error ? e.message : String(e),
      },
      { status: 500 }
    );
