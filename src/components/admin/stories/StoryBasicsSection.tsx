...
export default function StoryBasicsSection({
  ...
  const [coverUploadNote, setCoverUploadNote] = useState<string | null>(null);
  ...
  const onUploadCover = async (file: File) => {
    if (!canScopeToStory) {
      setCoverUploadError(
        'Set a valid story slug to upload under covers/<slug>/.'
      );
      return;
    }
    setCoverUploading(true);
    setCoverUploadError(null);
    setCoverUploadNote(null);
    try {
      const fd = new FormData();
      fd.append('file', file);
      fd.append('assetKind', 'cover');
      fd.append('storySlug', normalizedSlug);
      ...
      onChange({ ...form, coverUrl: data.fileUrl });
      setCoverUploadNote(
        `Uploaded: ${data.storagePath || data.fileUrl}`
      );
      setCoverScope('story');
      if (coverPickerOpen) {
        try {
      ...
      setCoverUploadError(e instanceof Error ? e.message : 'Upload failed');
    } finally {
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <div className="block sm:col-span-2">
          ...
          <p className="mt-1 text-[11px] text-slate-500">
            Upload a cover here or browse the public bucket. Files land under{' '}
            <code className="rounded bg-slate-100 px-1">
              covers/{canScopeToStory ? normalizedSlug : '<slug>'}/
            </code>{' '}
            (display WebP URL is filled automatically).
            {!canScopeToStory
              ? ' Set a valid story slug to upload under covers/<slug>/.'
              : null}
          </p>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              disabled={coverUploading || !canScopeToStory}
              title={
                !canScopeToStory
                  ? 'Set a valid slug to upload under covers/<slug>/'
                  : undefined
              }
              onClick={() => fileInputRef.current?.click()}
              className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-800 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
            >
          ...
          {coverUploadNote ? (
            <p className="mt-1 text-[11px] text-emerald-700">{coverUploadNote}</p>
          ) : null}
          {coverUploadError ? (
            <p className="mt-1 text-xs text-rose-700">{coverUploadError}</p>
          ) : null}
          {coverPickerOpen ? (
